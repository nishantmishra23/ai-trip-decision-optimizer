...
st.caption("Generate a custom day-by-day travel schedule powered by Google Gemini AI")
...
with st.container(border=True):
    st.subheader("Itinerary parameters", icon=":material/tune:", anchor=False)
...
if "current_itinerary" in st.session_state:
    ...
    st.subheader(f"Day-by-day schedule for {dest_name}", icon=":material/timeline:", anchor=False)
    ...
    for idx, day_data in enumerate(result.get("itinerary", [])):
        ...
        with st.container(border=True):
            ...
            for s_idx, slot in enumerate(slots):
                ...
                with c_slot1:
                    st.markdown(f":material/schedule: **{time_label}** — {title_label}")
                    if tip_label:
                        st.caption(f":material/lightbulb: *Tip:* {tip_label}")
                with c_slot2:
    ...
    m1, m2 = st.columns(2)
    with m1:
        st.metric("Total estimated trip cost", format_currency(total_trip_cost))
    with m2:
        st.metric("Cost per person", format_currency(total_trip_cost / max(trv, 1)))

    df_days = pd.DataFrame(daily_costs)
    if not df_days.empty:
        st.subheader("Daily cost distribution", icon=":material/bar_chart:", anchor=False)
        fig_daily = px.bar(
            df_days,
            x="Day",
            y="Estimated cost (₹)",
            text_auto=True,
            color="Day",
            color_discrete_sequence=["#4F46E5", "#0EA5E9", "#10B981", "#F59E0B", "#EF4444", "#8B5CF6", "#64748B"]
        )
