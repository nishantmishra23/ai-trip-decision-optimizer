...
st.title(":material/wb_sunny: Weather Intelligence", anchor=False)
...
img_url = get_destination_image(dest_name, selected_dest.get("category"))
st.image(img_url, caption=f"Climate Insights for {dest_name}, {selected_dest.get('country')}")

weather_data = get_weather(dest_name)

st.subheader(f":material/cloud: Live Weather Summary: {dest_name}", anchor=False)
...
with st.container(border=True):
    ...
    with c_cond:
        ...
        st.markdown(f"**Travel Suitability:** :green[Excellent]")
    with c_best:
